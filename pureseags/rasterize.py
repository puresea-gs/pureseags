...
def rasterize_gaussians(
    ...
    img_width: int,
    block_width: int,
    beta_per_gaussian: Optional[Float[Tensor, "*batch 1"]] = None,
    ...
    return _RasterizeGaussians.apply(
        ...
        img_width,
        block_width,
        beta_per_gaussian.contiguous() if beta_per_gaussian is not None else torch.empty(0, device=colors.device),
...
class _RasterizeGaussians(Function):
    ...
    def forward(
        ...
        img_width: int,
        block_width: int,
        beta_per_gaussian: Float[Tensor, "*batch 1"],
        ...
        if num_intersects < 1:
            ...
            use_pgb = beta_per_gaussian.numel() > 0
            out_img, out_clr, out_medium, depth_im, out_T, final_Ts, final_idx, first_idx = rasterize_fn(
                tile_bounds,
                block,
                img_size,
                gaussian_ids_sorted,
        ...
        ctx.use_per_gaussian_beta = use_pgb
        ...
        if return_alpha:
            ...
            return out_img, out_clr, out_medium, depth_im, out_T, out_alpha
        else:
            return out_img, out_clr, out_medium, depth_im, out_T

    @staticmethod
    def backward(ctx, v_out_img, v_out_clr, v_out_medium, v_depth_im, v_out_T, v_out_alpha=None):
        ...
        use_pgb = getattr(ctx, 'use_per_gaussian_beta', False)
        ...
        if num_intersects < 1:
            ...
            v_beta = torch.zeros_like(beta_per_gaussian) if use_pgb else None
            ...
            v_xy, v_conic, v_colors, v_opacity, v_depths, v_beta, v_medium_rgb, v_medium_bs, v_medium_attn = result
        ...
        return (
            ...
            None,  # 3: radii
            v_conic,  # 4: conics
            None,  # 5: num_tiles_hit
            v_colors,  # 6: colors
            v_opacity,  # 7: opacity
            v_medium_rgb,  # 8: medium_rgb
            v_medium_bs,  # 9: medium_bs
            v_medium_attn,  # 10: medium_attn
            None,  # 11: img_height
            None,  # 12: img_width
            None,  # 13: block_width
            v_beta,  # 14: beta_per_gaussian
            None,  # 15: background
            None,  # 16: return_alpha
            None,  # 17: step
        )
