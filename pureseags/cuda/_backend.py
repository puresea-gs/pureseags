...
try:
    ...
    from pureseags import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "pureseags_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "pureseags_cuda.lib")
        ):
            ...
            with Console().status(
                "[bold yellow]pureseags: Setting up CUDA (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]pureseags: No CUDA toolkit found. pureseags will be disabled.[/yellow]"
        )
